...
%% SIFT Operation
[class_descriptors_cnt, ~, ~, all_descriptors] = SIFT(train_folder_path, subfolder_names);


%% KMeans Clustering
cluster_cnt = 1000;
% cluster_idx_list = kmeans(input_x, cluster_num)
[~, centers] = kmeans(all_descriptors, cluster_cnt, 'Distance', 'cityblock', 'MaxIter', 3000);
...
for threshold = 0.9 : 0.01 : 1
    ...
    % SIFT operation
    [~, descriptors, class_image_cnt, ~] = SIFT(all_folder_path, subfolder_names);
